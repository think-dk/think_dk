CREATE TABLE `SITE_DB`.`item_event` (
  ...
  CONSTRAINT `item_event_ibfk_2` FOREIGN KEY (`backer_1`) REFERENCES `SITE_DB`.`users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `item_event_ibfk_3` FOREIGN KEY (`backer_2`) REFERENCES `SITE_DB`.`users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
