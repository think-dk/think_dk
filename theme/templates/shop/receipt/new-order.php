...
<? if($order): ?>
	...
	<div class="order_info">
		...
		<h2>Your order is confirmed <br />– payment is being processed</h2>
		...
		<p>
			You will receive an order confirmation on <em><?= $user["email"] ?></em> shortly.
		</p>
	...
	foreach($order["items"] as $order_item):
		...
		if($query->sql($sql)) {
			...
			if(!$has_ticket): ?>
			<h2>Your tickets</h2>
			<p>You will receive an email with your ticket(s), once the payment has been processed.</p>

			<h3>Tickets in this order:</h3>
			<ul class="ticket">
			...
			foreach($tickets as $key => $ticket): ?>
				<li class="ticket"><?= $order_item["name"] ?> - <?= $ticket["ticket_no"] ?><?= (count($tickets) > 1 ? " (".($key+1)."/".count($tickets).")" : "") ?></li>
			<? endforeach; ?>
