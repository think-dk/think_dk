...
<div class="scene i:scene defaultList <?= $itemtype ?>List">
	<h1>Tickets</h1>
	<h2>Ticket Admin list</h2>

	<ul class="actions">
		<?= $HTML->link("New ticket", "/janitor/ticket/admin-new", array("wrapper" => "li.new", "class" => "button primary")) ?>
	</ul>

	<ul class="tabs">
		<?= $HTML->link("Tickets for sale", "/janitor/ticket/admin-list", array("wrapper" => "li.".(!$expired ? "selected" : ""))) ?>
		<?= $HTML->link("Expired tickets", "/janitor/ticket/admin-list/expired", array("wrapper" => "li.".($expired ? "selected" : ""))) ?>
	</ul>
...
<? 			if($expired): ?>
				<?= $JML->listActions($item, [
					"modify" => [
						...
						"edit" => [
							"label" => "View",
							"url" => "/janitor/ticket/admin-view/".$item["id"]
						]
					]
				]) ?>
<?			else: ?>
				<?= $JML->listActions($item, ["modify" => [
					"edit" => [
						"url" => "/janitor/ticket/admin-edit/". $item["id"]
					],
				]]) ?>
<?			endif; ?>
