...
// debug([$membership, $price, $subscription["custom_price"]]);
...
<?						endif; ?>
	...
	<div class="changeprice item i:collapseHeader open i:customPrice">
		...
		<p>
			...
			Remember that supporting your local communities, can make a world of difference. With that said,
			we also fully understand that we are not alone in this situation, and that you may need the money 
			more than think.dk does. Make the choice that is right for you.
		</p>
		<p>
			Enter the price you can afford below and click the <em>Update membership price</em> button.
		</p>

		<?= $model->formStart("/janitor/admin/member/updateMembershipPrice/".$user_id, array("class" => "customprice labelstyle:inject")) ?>
			<?= $model->input("item_id", ["type" => "hidden", "value" => $membership["item_id"]]) ?>

			<fieldset>
				<?= $model->input("custom_price", ["value" => ($custom_price ? $custom_price["price"] : $price["price"])]) ?>
			</fieldset>

			<ul class="actions">
				<?= $model->submit("Update membership price", array("class" => "primary key:s", "wrapper" => "li.update")) ?>
			</ul>
		<?= $model->formEnd() ?>
